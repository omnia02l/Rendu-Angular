...
export class ApartmentsComponent {
  ...
  ngOnInit() {
    this.idResidence = Number(this.actR.snapshot.paramMap.get('idR'));
    this.listApartments = this.getApartmentsByResidence();
    this.filteredApartments = this.listApartments;
  }

  getApartmentsByResidence(): Apartment[] {
    return this.listApartments.filter(ap => ap.residence.id === this.idResidence);
  }

  likeApartment(apartment: Apartment): void {
    if (!this.likedApartments.includes(apartment)) {
      this.likedApartments.push(apartment);
    }
  }

  filterApartmentsBySurface(): void {
    if (this.surfaceFilter !== undefined) {
      this.filteredApartments = this.listApartments.filter(apartment => apartment.surface === this.surfaceFilter);
    } else {
      this.filteredApartments = this.listApartments; 
    }
  }
